...
TEST_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bixel-subscription-tests.XXXXXX")"
...
xcrun swiftc -module-name BixelSubscriptionTests \
    ...
    "${SOURCES[@]}" "$ROOT/tests/SubscriptionEntitlementTests.swift" -o "$TEST_DIR/tests"
