...
cd "$ROOT"
TEST_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bixel-interaction-tests.XXXXXX")"
trap 'rm -rf "$TEST_DIR"' EXIT
SOURCES=()
while IFS= read -r source; do
    [[ "$source" == */BixelApp.swift ]] || SOURCES+=("$source")
done < <(rg --files app/Bixel -g '*.swift')
xcrun swiftc -module-name BixelInteractionTests \
    ...
    "${SOURCES[@]}" tests/EditorInteractionTests.swift -o "$TEST_DIR/tests"
"$TEST_DIR/tests"
